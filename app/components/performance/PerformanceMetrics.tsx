...
export default function PerformanceMetrics() {
  const { loading, best, worst, trend } = usePerformanceMetrics();

  if (loading || !best || !worst || !trend) {
    ...
    <div className="bg-white border border-[#DCE3F1] rounded-[14px] p-6 shadow-[0_2px_6px_rgba(59,130,246,0.08)] flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="flex items-center gap-3 p-4 bg-[#F8FAFF] rounded-[14px] border border-[#DCE3F1]">
          ...
          <span className="text-[15px] text-[#334155] font-semibold">
            {best.day}
          </span>
          <span className="text-[17px] font-bold text-[#22C55E]">
            ${best.revenue}
          </span>
        </div>
        ...
        <div className="flex items-center gap-3 p-4 bg-[#F8FAFF] rounded-[14px] border border-[#DCE3F1]">
          ...
          <span className="text-[17px] font-bold text-[#EF4444]">
            ${worst.revenue}
          </span>
        </div>
